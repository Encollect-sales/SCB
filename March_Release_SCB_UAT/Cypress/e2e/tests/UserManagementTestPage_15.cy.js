...
describe('UserManagement - ENCollect system should throw an error as "Invalid Action" when user enters any other value apart from enable/disable in action column.', () => {
    ...
    it('User download the agent template and make updates in the excel sheet and user upload the updated excel sheet - TC_ID_15', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            cy.wait(900);
            usermanagementpage.UserManagementTestPage_download_15();
            ...
            const data = {
                ...
                usermanagementpage.UserManagementTestPage_upload_15();
            });
        });
    });
