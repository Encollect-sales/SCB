...
describe('TC_344_POS - Verify Bulk Payments Upload menu', () => {
    let loginPage;
    ...
    it('Verify Bulk Payments Upload menu is available under Payments module', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyBulkPaymentsUploadMenuAvailable();
        });
    });
});