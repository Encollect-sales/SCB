...
describe('TC_339_POS - Verify Send Payment Link option is available', () => {
    let loginPage;
    ...
    it('Verify Send Payment Link option is available in action menu', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyAdditionalDetailsSendPaymentLinkOption();
        });
    });
});