...
describe('TC_326_NEG - Verify all Product Information fields are empty', () => {
    let loginPage;
    ...
    it('Verify behavior when all fields have empty values', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyAllProductInformationFieldsEmpty();
        });
    });
});