...
describe('TC_294_NEG - Verify all Basic Information fields empty', () => {
    let loginPage;
    ...
    it('Verify behavior when all fields have empty values and Hide Empty Values is enabled', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyBasicInformationAllFieldsEmpty();
        });
    });
});