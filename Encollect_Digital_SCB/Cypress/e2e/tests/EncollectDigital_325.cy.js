...
describe('TC_325_NEG - Verify no Product Information field mapping', () => {
    let loginPage;
    ...
    it('Verify behavior when no field mapping configuration exists', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyNoProductInformationFieldMapping();
        });
    });
});