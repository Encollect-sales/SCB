...
describe('TC_251_POS - Verify Vendor Request ID column is displayed', () => {
    let loginPage;
    ...
    it('Verify Vendor Request ID column is displayed', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            //receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyVendorRequestIdColumn();
        });
    });
});