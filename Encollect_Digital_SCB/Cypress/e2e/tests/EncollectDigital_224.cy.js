...
describe('TC_224_POS - Verify Trail History records', () => {
    let loginPage;
    ...
    it('Verify Trail History records are displayed in tabular format', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            accountDetailsPage.navigateToAccountDetails();
            accountDetailsPage.verifyTrailHistoryRecordsDisplayed();
        });
    });
});