...
describe('TC_255_POS - Verify settings gear icon is displayed', () => {
    let loginPage;
    ...
    it('Verify settings gear icon is displayed', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyCommunicationHistorySettingsIconDisplayed();
        });
    });
});