...
describe('TC_274_POS - Verify settings displays available fields', () => {
    let loginPage;
    ...
    it('Verify Payment History settings displays available fields', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyPaymentHistorySettingsFields();
        });
    });
});