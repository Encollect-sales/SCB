...
describe('TC_229_POS - Verify PTP Date / Next Action Date column', () => {
    let loginPage;
    ...
    it('Verify PTP Date / Next Action Date column is displayed', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            accountDetailsPage.navigateToAccountDetails();
            accountDetailsPage.verifyPTPNextActionDateColumnDisplayed();
        });
    });
});