...
describe('TC_324_POS - Verify Send Payment Link functionality', () => {
    let loginPage;
    ...
    it('Verify Send Payment Link functionality', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyProductInformationSendPaymentLinkFunctionality();
        });
    });
});