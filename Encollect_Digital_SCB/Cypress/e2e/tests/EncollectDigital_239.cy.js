...
describe('TC_239_NEG - Verify no trail history records', () => {
    let loginPage;
    ...
    it('Verify behavior when no trail history records are available', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.verifyNoTrailHistoryRecords();
        });
    });
});