...
describe('TC_277_NEG - Verify no payment history records', () => {
    let loginPage;
    ...
    it('Verify behavior when no payment history records are available', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyNoPaymentHistoryRecords();
        });
    });
});