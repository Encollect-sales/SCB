...
describe('TC_345_POS - Verify Bulk Payments Upload Status menu', () => {
    let loginPage;
    ...
    it('Verify Bulk Payments Upload Status menu is available under Payments module', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
            receivableSearchPage.navigateToAccountDetails();
            receivableSearchPage.verifyBulkPaymentsUploadStatusMenuAvailable();
        });
    });
});