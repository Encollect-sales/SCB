...
describe('ScopeofWork - user will click on Collection Intensity report and check the scoped Branch', () => {
    let loginPage;
    let scopeofworkpage; 
    ...
    it('ScopeofWork- TC_ID_040', () => {
        getTestData('loginData', 'login1').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(7000);
                scopeofworkpage.ScopeofWorkTestPage_040();            
            });
        });
    });