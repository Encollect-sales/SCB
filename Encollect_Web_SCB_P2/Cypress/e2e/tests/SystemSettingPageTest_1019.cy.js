...
describe('System Setting - Account Import Via Front End Bulk Upload (Credit Card)', () => {
    ...
    it('System Setting - TC_ID_1019 - Admin to check the uploaded Account import file status by clicking on "upload account import status" menu and by selecting anyone of the filter values', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                //systemsettingPage.systemSettingModule();
                systemsettingPage.AccountImportViaFrontEndBulkUploadCreditCard_1019();
                //loginPage.logout();
