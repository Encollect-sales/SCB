...
describe('System Setting - Account Import Via Front End Bulk Upload (Loans)', () => {
    ...
    it('System Setting - TC_ID_988 - Admin clicked on upload account import status menu to find out the status of uploaded account import file but user clicked on search button without selecting anyone of the filters', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                systemsettingPage.systemSettingModule();
                systemsettingPage.AccountImportViaFrontEndBulkUploadLoan_988();
