...
describe('System Setting - Account Import Via Front End Bulk Upload (Credit Card)', () => {
    ...
    it('System Setting - TC_ID_1032 - Admin entered 1000 records in standard account import file with all mandatory data and the file size is greater than 3 MB', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                systemsettingPage.SystemSettingTestPage1032();
                //loginPage.logout();
