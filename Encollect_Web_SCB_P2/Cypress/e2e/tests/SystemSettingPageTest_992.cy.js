...
describe('System Setting - Account Import Via Front End Bulk Upload (Loans)', () => {
    ...
    it('System Setting - TC_ID_992 - Admin to check the uploaded Account import file status by clicking on "upload account import status" menu and by selecting anyone of the filter values', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                systemsettingPage.Systemsetting992();
                //loginPage.logout();
