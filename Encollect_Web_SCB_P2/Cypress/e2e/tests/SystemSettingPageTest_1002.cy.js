...
describe('System Setting - Account Import Via Front End Bulk Upload (Loans)', () => {
    ...
    it('System Setting - TC_ID_1002 - Admin entered 1000 records in standard account import file but agreement not updated only for 500 records and the file size is 3MB(Threshold)', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                systemsettingPage.systemSettingModule();
                systemsettingPage.AccountImportViaFrontEndBulkUploadLoan_1002();
