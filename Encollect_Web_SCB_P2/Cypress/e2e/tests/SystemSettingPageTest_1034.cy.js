...
describe('System Setting - Account Import Via Front End Bulk Upload (Credit Card)', () => {
    ...
    it('System Setting - TC_ID_1034 - Admin to upload Account import file from frontend by clicking on System setting >>Account import master>>Upload Account import master(Next Day)', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                systemsettingPage.Systemsetting1034();
                //loginPage.logout();
