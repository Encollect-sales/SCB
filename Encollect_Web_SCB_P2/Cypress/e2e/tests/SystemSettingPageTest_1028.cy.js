...
describe('System Setting - Account Import Via Front End Bulk Upload (Credit Card)', () => {
    ...
    it('System Setting - TC_ID_1028 - Admin entered 1000 records in standard account import file but agreement not updated for all the records and the file size is 3MB(Threshold)', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(5000);
                systemsettingPage.SystemSettingTestPage1028();
                //loginPage.logout();
