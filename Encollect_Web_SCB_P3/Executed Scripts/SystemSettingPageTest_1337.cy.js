...
describe('System Setting - Master Upload', () => {
    ...
    it('Deposit Bank Master - TC_ID_1337 - Admin after uploading the deposit bank master file back to ENCollect admin should receive a notification on email id with the status.', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(500);
                systemsettingPage.systemSettingModule();
                systemsettingPage.DepositeBankMaster1337();
