...
describe('System Setting - Master Upload', () => {
    ...
    it('Base Branch Master - TC_ID_1146 - Admin downloaded the standard base branch master file but did not entered any data and uploaded the file back to ENCollect', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(900);
                systemsettingPage.systemSettingModule();
                systemsettingPage.BaseBranchMaster1146();
