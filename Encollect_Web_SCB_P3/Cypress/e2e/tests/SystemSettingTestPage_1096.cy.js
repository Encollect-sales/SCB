...
describe('System Setting - Master Upload - Admin after uploading the product master file back to ENCollect admin should receive a notification on email id with the status.', () => {
    ...
    it('Master Upload- TC_ID_1096-  An email will trigger on admin email id with an attachment.', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(900);
                systemsettingpage.SystemSettingTestPage_1096();
    ...
    cy.task('updateProductMasterTemplateCell', {
      ...
      systemsettingpage.SystemSettingTestPage_file_upload_1096();
