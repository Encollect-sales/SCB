...
describe('System Setting - Master Upload', () => {
    ...
    it('Deposit Bank Master - TC_ID_1336 - Admin downloaded the standard deposit bank master file but did not entered any data and uploaded the file back to ENCollect', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(900);
                systemsettingPage.systemSettingModule();
                systemsettingPage.DepositeBankMaster1336();
