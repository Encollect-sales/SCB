...
describe('System Setting - Master Upload', () => {
    ...
    it('Bank Master - TC_ID_1320 - Admin should not change the header formats or should not add new fields in the standard bank master file while uploading back to ENCollect', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(900);
                systemsettingPage.systemSettingModule();
                systemsettingPage.BankMaster1320();
